const JT = require('../../libs/jstween').TweenMax
class Cat {
  constructor () {
  ...
  init () {
    ...
    this.instance.add(this.rightShoulder);
    this.instance.add(this.leftShoulder);

    this.instance.add(this.rightKnee);
    this.instance.add(this.leftKnee);
    this.instance.add(this.tail);
  ...
  updateArm (side, angles, speed, ease, callBack){
    ...
    JT.to(shoulder.rotation, speed, {y:angles.theta, ease:ease} );
    JT.to(arm.rotation, speed, {x:angles.alpha, ease:ease} );
    JT.to(foreArm.rotation, speed, {x:angles.beta, ease:ease, onComplete:callBack} );
    JT.to(foot.rotation, speed, {x:tFootAngle, ease:ease} );
  }
  blink (){
    let _this = this;
    JT.to (this.rightEye.scale, .07, {y:0, yoyo:true, repeat:1});
    JT.to (this.leftEye.scale, .07, {y:0, yoyo:true, repeat:1, onComplete:function(){
      _this.isBlinking = false;
    }});
  }
  moveMouth (){
    let _this = this;
    JT.to (this.mouth.rotation, .2, {x:Math.PI/6, yoyo:true, repeat:1, onComplete:function(){
